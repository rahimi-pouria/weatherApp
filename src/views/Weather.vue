...
let getName = ref<string>('');
  ...
  let day = ref()
    ...
    const dataCity = ref<cityNameData[]>([
            {
              "city": "Tehran", 
              "lat": "35.7000", 
              "lng": "51.4167", 
    ...
    async function getWeather(){
        getName.value = nameCity.value;
        if(nameCity.value === '') {
            alert('پر کردن فیلدها الزامی می باشد')
    } else {
        console.log(getName.value)
        for (let i = 0; i < dataCity.value.length; i++) { 
          if (dataCity.value[i].city === getName.value) {
              latitude.value = dataCity.value[i].lat
              lngs.value = dataCity.value[i].lng
              await axios.get(`https://api.open-meteo.com/v1/forecast?latitude=${latitude.value}&longitude=${lngs.value}&current_weather=true`)
              .then((response) => { 
                dataWeatherCity = response
                  console.log(dataWeatherCity)
                  temperature = dataWeatherCity.data.current_weather.temperature
                  day = dataWeatherCity.data.current_weather.is_day
                })
          }
        }
            nameCity.value = ''
